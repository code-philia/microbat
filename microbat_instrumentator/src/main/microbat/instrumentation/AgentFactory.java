...
import microbat.instrumentation.ondemandtrace.OnDemandTraceAgent;
...
//		instrumentation = inst;
		...
		if (cmd.getBoolean(CoverageAgentParams.OPT_IS_COUNT_COVERAGE, false)) {
			...
			agent.setInstrumentation(inst);
		} else if (cmd.getBoolean(AgentParams.OPT_ONDEMAND_TRACE, false)) {
			agent = new OnDemandTraceAgent(cmd);
			agent.setInstrumentation(inst);
		} else {
