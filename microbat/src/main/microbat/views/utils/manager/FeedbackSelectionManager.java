...
public class FeedbackSelectionManager {
	...
	public void verify(UserFeedback[] allAvaiableFeedbacks) {
		if (this.feedbackYesNoButtonsMap.size() != allAvaiableFeedbacks.length) {
			throw new RuntimeException(
					Log.genMsg(getClass(), "YesNoButtonPairs count mismatch with number of feedback"));
		}
		...
		for (Map.Entry<UserFeedback, YesNoButtonPair> entry : this.feedbackYesNoButtonsMap.entrySet()) {
			...
			if (!pair.isValid()) {
				throw new RuntimeException(
						Log.genMsg(getClass(), "Button pair is not valid for feedback: " + feedback));
			}
		}
	}
	 ...
	 * 1. There is at least one wrong feedback (root cause, wrong path, wrong
	 * variable) or <br/>
	...
	public void checkButtonBasedOnFeedback(final DPUserFeedback feedback) {
		...
		case CORRECT:
			/*
			 * When feedback is correct 1. This step is not root cause 2. This step should
			 * be executed 3. All variables are correct
			 */
			for (Map.Entry<UserFeedback, YesNoButtonPair> entry : this.feedbackYesNoButtonsMap.entrySet()) {
		...
		case ROOT_CAUSE:
			/*
			 * When feedback is root cause 1. This step is not correct 2. Unclear with all
			 * other feedback
			 */
			for (Map.Entry<UserFeedback, YesNoButtonPair> entry : this.feedbackYesNoButtonsMap.entrySet()) {
				final UserFeedback feedbackKey = entry.getKey();
		...
		case WRONG_PATH:
			/*
			 * When feedback is wrong path 1. This step is not root cause 2. This step is
			 * not correct 3. Unclear with variables
			 */
			for (Map.Entry<UserFeedback, YesNoButtonPair> entry : this.feedbackYesNoButtonsMap.entrySet()) {
				final UserFeedback feedbackKey = entry.getKey();
				final YesNoButtonPair pair = entry.getValue();
		...
		case WRONG_VARIABLE:
			/*
			 * When feedback is wrong variable 1. This step is not root cause 2. This step
			 * is in correct branch 3. This step is not correct
			 */
			for (Map.Entry<UserFeedback, YesNoButtonPair> entry : this.feedbackYesNoButtonsMap.entrySet()) {
				final UserFeedback feedbackKey = entry.getKey();
				final YesNoButtonPair pair = entry.getValue();
	...
	public void select(final UserFeedback userFeedback, final boolean isYesButton) {
		...
		if (feedbackType.equals(UserFeedback.ROOTCAUSE)) {
			...
			// If user select wrong path, no matter it is wrong or correct, that mean this
			// step is not root cause
			YesNoButtonPair pair = this.feedbackYesNoButtonsMap.get(userFeedback);
			this.feedbackYesNoButtonsMap.get(this.genRootCuaseFeedbackKey()).checkNoButton();
			if (pair.isNoButtonSelected()) {
				// If user say it is wrong path, then this step is wrong and all variable is
				// unclear
				for (Map.Entry<UserFeedback, YesNoButtonPair> entry : this.feedbackYesNoButtonsMap.entrySet()) {
					final UserFeedback feedbackKey = entry.getKey();
					final YesNoButtonPair buttonPair = entry.getValue();
					if (feedbackKey.getFeedbackType().equals(UserFeedback.CORRECT)
							|| feedbackKey.getFeedbackType().equals(UserFeedback.ROOTCAUSE)) {
			...
			// If user select wrong variable, that mean this step is not root cause and
			// should be executed
			YesNoButtonPair pair = this.feedbackYesNoButtonsMap.get(userFeedback);
			if (pair.isNoButtonSelected()) {
				for (Map.Entry<UserFeedback, YesNoButtonPair> entry : this.feedbackYesNoButtonsMap.entrySet()) {
		...
		} else if (feedbackType.equals(UserFeedback.CORRECT)
				&& this.feedbackYesNoButtonsMap.get(userFeedback).isYesButtonSelected()) {
