...
public class VariableSkeleton {
	...
	public VarValue toVarValue(VarValue source) {
		...
		List<VarValue> children = this.children.stream()
				.map(v -> v.toVarValue(source.getAliasVarID(), source.getVarID())).toList();
		varValue.setChildren(children);
		return varValue;
	}

	private VarValue toVarValue(String aliasID, String varID) {
		...
		List<VarValue> children = this.children.stream().map(v -> v.toVarValue(newAliasID, newVarID)).toList();
		varValue.setChildren(children);
		return varValue;
	}
