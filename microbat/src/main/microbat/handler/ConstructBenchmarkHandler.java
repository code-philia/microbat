...
//						generateDefNegtiveExample(mainTrace);
	...
	private void generateDefBenchmark(Trace trace) {
		...
		for(int i = 0;i<trace.getExecutionList().size();i++) {
			...
			for(VarValue targetVar : step.getWrittenVariables()) {
				...
				VarValue rootVar = targetVar; // here "rootVar" is a pointer, and finally it points to the actual rootVar
				Set<VarValue> visited = new HashSet<VarValue>();
				visited.add(targetVar);
				
				/*
				 * starting at targetVar, find its parent, until reach to a variable that has no parent -> rootVar
				 */
				while(true) {
					boolean found = false;
        ...
        try {
            Files.write(Paths.get(BENCHMARK_FOLDER_PATH,className+".json"), jsonArray.toString(4).getBytes());
	...
	private void generateDefBenchmark_Neg(Trace trace) {
        ...
        try {
            Files.write(Paths.get(BENCHMARK_FOLDER_PATH,className+"_Neg.json"), jsonArray.toString(4).getBytes());
            System.out.println("JSON file created successfully.");
        } catch (IOException e) {
            System.out.println("An error occurred while writing JSON to file: " + e.getMessage());
        }
	...
	public void generateDefPositiveExample(Trace trace) {
		...
		for(int i = 0;i<trace.getExecutionList().size();i++) {
			...
			for(VarValue targetVar : step.getWrittenVariables()) {
	            ...
	            try {
					Files.write(Paths.get(EXAMPLE_FILE), result.getBytes(), 
	...
	public HashMap<String,String> getBenchmarkSample(TraceNode step, VarValue rootVar, VarValue targetVar, String fullFieldName, boolean written){
		HashMap<String,String> e = new HashMap<>();
		/* source code */
