...
public class ComponentEvaluationHandler extends StartDebugHandler {
	...
	private final String BENCHMARK_PATH = "C:\\Users\\Kwy\\Desktop\\RQ2\\RQ2_benchmark";
	private final String BENCHMARK_DEF_PATH = "C:\\Users\\Kwy\\Desktop\\RQ2\\RQ2_benchmark_def";
	private final String RESULT_PATH = "C:\\Users\\Kwy\\Desktop\\RQ2\\RQ2_result_def";

	private static ArrayList<String> classes = new ArrayList<>(Arrays.asList("leetbugs.ANAGRAM", "leetbugs.ANAGRAMS",
			"leetbugs.BINARY", "leetbugs.BUCKETSORT", "leetbugs.CAMAL", "leetbugs.COUNT_SAY", "leetbugs.DECODE",
			"leetbugs.DETECT_CYCLE", "leetbugs.DISTINCT", "leetbugs.DUPLICATE", "leetbugs.EVAL_RPN",
			"leetbugs.FREQUENCY", "leetbugs.GET_FACTORS", "leetbugs.INTERSECT", "leetbugs.ISOMORPHIC",
			"leetbugs.KHEAPSORT", "leetbugs.LETTERS", "leetbugs.LEVEL", "leetbugs.LIS", "leetbugs.MERGE_SIMILAR_ITEM",
			"leetbugs.MISSING_NUM", "leetbugs.PASCAL", "leetbugs.PATH", "leetbugs.POWERSET", "leetbugs.SPIRAL",
			"leetbugs.SUBARRAY", "leetbugs.THREENUM", "leetbugs.UGLY_NUM"));
...
//		evaluateVarExpansionWrapper(appClassPath, trace);
		evaluateDefInferenceWrapper(appClassPath, trace);
	}

	protected void evaluateVarExpansionWrapper(AppJavaClassPath appClassPath, Trace trace) {
		String className = Settings.launchClass.substring(Settings.launchClass.indexOf(".") + 1);
		ObjectMapper objectMapper = new ObjectMapper();
		Path bmPath = Paths.get(BENCHMARK_PATH, className + ".json");
		...
		String result = evaluateVarExpansion(appClassPath, trace, benchmark);
	...
	protected String evaluateVarExpansion(final AppJavaClassPath appClassPath, Trace trace,
			List<Map<String, String>> benchmark) {
		// var-expansion
	...
	protected void evaluateDefInferenceWrapper(AppJavaClassPath appClassPath, Trace trace) {
		// preparation for evaluating def_infer
		String className = Settings.launchClass.substring(Settings.launchClass.indexOf(".") + 1);
		ObjectMapper objectMapper = new ObjectMapper();
		Path bmPath1 = Paths.get(BENCHMARK_DEF_PATH, className + ".json");
		Path bmPath2 = Paths.get(BENCHMARK_DEF_PATH, className + "_Neg.json");
	...
	protected String evaluateDefInference(final AppJavaClassPath appClassPath, Trace trace,
			List<Map<String, String>> benchmark) {
		// def-inference
		ArrayList<Map<String, String>> failed = new ArrayList<>();
		ArrayList<Map<String, String>> successful = new ArrayList<>();
		...
		for (Map<String, String> element : benchmark) {
			...
			Condition condition = new Condition(rootVar.getVarName(), rootVar.getType(), rootVar.getStringValue(),
					"null");
			...
			element.put("RootVarType", rootVar.getType());
			element.put("RootVarValue", rootVar.getStringValue());
			if (def == gt) {
		...
		result.append("Successful: " + successful.size() + "\n");
		result.append("Failed: " + failed.size() + "\n");

		double def_infer_rate = (double) successful.size() / (successful.size() + failed.size());
		result.append("Successful rate: " + def_infer_rate + "\n");

        ObjectMapper mapper = new ObjectMapper();
        try {
            String skipped_json = mapper.writeValueAsString(skipped);
            String successful_json = mapper.writeValueAsString(successful);
            String failed_json = mapper.writeValueAsString(failed);

    		result.append("[Skipped]:" + skipped_json + "\n");
    		result.append("[Successful]:"+successful_json+"\n");
    		result.append("[Failed]:" + failed_json + "\n");
